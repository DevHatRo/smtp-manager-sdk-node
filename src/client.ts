...
/** The hosted platform. Self-hosted instances pass their own `baseUrl`. */
export const DEFAULT_BASE_URL = 'https://api.your-email.eu';
...
function resolveConfig(options: SmtpManagerOptions): ResolvedConfig {
  ...
  if (baseUrl !== undefined && (typeof baseUrl !== 'string' || baseUrl === '')) {
    throw new ConfigurationError('baseUrl must be a non-empty string when given');
  }
  ...
  return {
    apiKey,
    messagesUrl: resolveMessagesUrl(baseUrl ?? DEFAULT_BASE_URL),
    timeoutMs,
    maxRetries,
    maxRetryAfterMs,
