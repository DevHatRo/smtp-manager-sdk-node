...
export interface SmtpManagerOptions {
  ...
  apiKey: string;
  /**
   * The Endpoint the console shows with a new key (`https://host/api/v1/messages`) or the bare
   * origin (`https://host`, optionally with a path prefix). `http://` is accepted for local
