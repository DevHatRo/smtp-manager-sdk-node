...
describe('SmtpManager configuration', () => {
  it('exposes the defaults', () => {
    expect(DEFAULT_BASE_URL).toBe('https://api.your-email.eu');
  ...
  it.each([
    ...
    [{ apiKey: API_KEY, baseUrl: '' }, 'baseUrl must be a non-empty string when given'],
    [{ apiKey: API_KEY, baseUrl: 42 }, 'baseUrl must be a non-empty string when given'],
    [{ apiKey: API_KEY, baseUrl: null }, 'baseUrl must be a non-empty string when given'],
  ...
  it('defaults baseUrl to the hosted platform', async () => {
    expect(new SmtpManager({ apiKey: API_KEY }).messagesUrl).toBe(
      'https://api.your-email.eu/api/v1/messages',
    );
    expect(new SmtpManager({ apiKey: API_KEY, baseUrl: undefined }).messagesUrl).toBe(
      'https://api.your-email.eu/api/v1/messages',
    );
    expect(JSON.parse(JSON.stringify(new SmtpManager({ apiKey: API_KEY }))).messagesUrl).toBe(
      'https://api.your-email.eu/api/v1/messages',
    );

    const fetch = fetchMock().mockResolvedValueOnce(accepted());
    await new SmtpManager({ apiKey: API_KEY, fetch }).messages.send({
      from: 'a@example.com',
      to: 'b@example.net',
      subject: 'S',
      text: 'T',
    });
    expect(lastCall(fetch).url).toBe('https://api.your-email.eu/api/v1/messages');
  });
