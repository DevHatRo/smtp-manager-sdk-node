...
const client = new SmtpManager({
  ...
  baseUrl: env('SMTP_MANAGER_BASE_URL'), // the console's Endpoint, or just https://host
});
...
| ------------------------- | ------------------------------------------------------------------------------------- | ------------------------------- |
| `ConfigurationError`      | `apiKey` missing / not `smtpk_…`, `baseUrl` missing / not http(s), bad numeric option | –                               |
...
Paste the Endpoint as-is into `baseUrl` (`https://host/api/v1/messages`); the bare origin
(`https://host`) works too — the SDK completes either form and never doubles the path.
...
The integration suite sends a real message and is skipped unless all four variables are set. In
CI it runs only on a manual `workflow_dispatch` of the CI workflow, never on pushes to `main`:

| Variable                | Meaning                                                    |
| ----------------------- | ---------------------------------------------------------- |
| `SMTP_MANAGER_BASE_URL` | The console's Endpoint or the origin, e.g. `https://smtp.example.com` |
